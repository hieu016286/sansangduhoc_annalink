...
$bunyad = Bunyad::core()->init(apply_filters('bunyad_init_config', array(
	...
	'page_builder_blocks' => array(
		...
		'news-faq' => 'Bunyad_PageBuilder_NewsFaq',
		'focus-grid' => 'Bunyad_PageBuilder_FocusGrid',
		'news-feature' => 'Bunyad_PageBuilder_NewsFeature',
...
// function to display number of posts.
	function getPostViews($postID){
		...
		if($count==''){
			...
			add_post_meta($postID, $count_key, '200');
			return '';//"200 View";
		}
	...
	function getAdminPostViews($postID){
		$count_key = 'post_views_count';
		$count = get_post_meta($postID, $count_key, true);
		if($count==''){
			delete_post_meta($postID, $count_key);
	...
	function posts_custom_column_views($column_name, $id){
	    if($column_name === 'post_views_count'){
	        echo getAdminPostViews(get_the_ID());
	    }
	}
...
add_action( 'admin_notices', function () {
    ...
    if ( 'publish' !== get_post_status( $post->ID ) && ! has_post_thumbnail( $post->ID ) ) { ?>
        <div id="message" class="error">
            <p>
                <strong><?php _e( 'Hãy chọn một ảnh tiêu biểu cho bài viết.' ); ?></strong>
            </p>
        </div>
    <?php
    }
    if ( 'publish' !== get_post_status( $post->ID ) && empty( $post->post_excerpt) ) { ?>
        <div id="message" class="error">
            <p>
                <strong><?php _e( 'Hãy điền tóm tắt cho bài viết.' ); ?></strong>
            </p>
        </div>
    <?php
    }
...
function my_post_image_html( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
  ...
  $AWSBucket = 'https://s3.ap-southeast-1.amazonaws.com/sansangduhoc.vn/wp-content/uploads';
...
function set_upload_folder( $upload_data ) { 
  ...
  $upload_data['baseurl'] = 'https://s3.ap-southeast-1.amazonaws.com/sansangduhoc.vn/wp-content/uploads';
...
class Bunyad_Theme_SmartMag
{
	...
	public function theme_init()
	{
		...
		$image_sizes = apply_filters('bunyad_image_sizes', array(
			...
			'main-block' => array('width' => 318, 'height' => 185),
			...
			'gallery-block' => array('width' => 536, 'height' => 386),
	...
	public function register_sidebars()
	{
		...
		/*
		register_sidebar(array(
			'name' => __('Sub Categrory', 'bunyad-admin'),
			'id'   => 'sub-category',
			'description' => __('Prefer simple text widgets here.', 'bunyad-admin'),
			'before_title' => '<h3 class="widgettitle">',
			'after_title'  => '</h3>',
	...
	public function setup_shortcodes()
	{
		...
		Bunyad::options()->shortcodes->add_blocks(array(
			...
			'news_faq' => array('render' => locate_template('blocks/news-faq.php'), 'attribs' => array(
				'posts' => 5, 'cat' => null, 'column' => '', 'tax_tag' => '', 'sub_cats' => '', 'sub_tags' => '',
				'sort_by' => '', 'sort_order' => '', 'highlights' => 1, 'taxonomy' => '', 'offset' => '', 'post_type' => '',
				'title' => '', 'heading_type' => 'block-filter'
			)),
			
			'news_feature' => array('render' => locate_template('blocks/news-feature.php'), 'attribs' => array(
				'posts' => 5, 'cat' => null, 'column' => '', 'tax_tag' => '', 'sub_cats' => '', 'sub_tags' => '',
				'sort_by' => '', 'sort_order' => '', 'highlights' => 1, 'taxonomy' => '', 'offset' => '', 'post_type' => '',
				'title' => '', 'heading_type' => 'block-filter'
			)),
