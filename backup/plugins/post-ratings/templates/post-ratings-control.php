...
<div class="ratings <?php if(is_singular()) print 'hreview-aggregate'; ?>" data-post="<?php the_ID(); ?>">
  ...
  <div class="meta">
     <?php
       printf(_n('%1$s đánh giá, điểm trung bình: %2$s', '%1$s đánh giá, điểm trung bình: %2$s', $votes, 'post-ratings'),
         sprintf('<strong class="votes">%d</strong>', $votes), sprintf('<strong>%.2F</strong>', $rating));
     ?>
  </div>
