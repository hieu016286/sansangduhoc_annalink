		...
		(function() {
			...
			if(tmp){
				...
				if (hook)
				{
					...
					var new_mceInit = {};
					new_mceInit['qtrans_textarea_content'] = hook;
					for ( edId in tinyMCEPreInit.mceInit ) {
						new_mceInit[edId] = tinyMCEPreInit.mceInit[edId];
					}
					tinyMCEPreInit.mceInit = new_mceInit;
				...
				ed.on('SaveContent', function(e) {
					if (!ed.isHidden()) {
						...
						if ( ed.getParam( 'wpautop', true ) )
							...
							jQuery('.ht-multilanguage-input').each(function(i,e){
							content = '';
	                        for(i=0; i < languages.length; i++){
	                        	var get_textarea = jQuery(e).parent().find('textarea#'+languages[i]+'_'+jQuery(e).attr('id')+'[aria-hidden=true]');
	                        	if(get_textarea.length > 0){
	                        		val = jQuery(e).parent().find('iframe#'+languages[i]+'_'+jQuery(e).attr('id')+'_ifr').contents().find('body').html();
									if(jQuery.trim(val) != ''){
		                            	content += '<!--:'+languages[i]+'-->'+val+'<!--:-->';
		                           	}
	                        	} else {
	                        		val = jQuery(e).parent().find('textarea#'+languages[i]+'_'+jQuery(e).attr('id')).val();
	                        		if(jQuery.trim(val) != ''){
		                        		content += '<!--:'+languages[i]+'-->'+val+'<!--:-->';
		                        	} 
	                        	}                           	
