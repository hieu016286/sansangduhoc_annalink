...
class cets_EmbedGmaps {
	...
	// This is the tiny mce button stuff
		$types = array(
			...
			__('Please enter code embed the Google Map', 'cets_EmbedGmaps'),
	...
	<?php echo $datajs; ?>
		...
		function cets_GEButtonOkay() {
			...
			var iframe = jQuery(jQuery.parseHTML(text));
			...
			if(iframe.attr('src').length > 0)
				text = iframe.attr('src');
			if(iframe.attr('width').length > 0)
				width = iframe.attr('width');
			if(iframe.attr('height').length > 0)
				height = iframe.attr('height');
			if(iframe.attr('frameborder').length > 0)
