...
?>
        <style type="text/css">
            ...
            .ht-multilanguage-input{display: none}
			.ht-multilanguage-form .language-form{display: none}
			.ht-multilanguage-form .language-form.current{display: block}
			.ht-multilanguage-form .switch-language-input{margin: 0;}
			.ht-multilanguage-form .switch-language-input .bnt-language{display: inline-block; margin-right: 5px; cursor: pointer;}
			.ht-multilanguage-form .btn-language-form{opacity: 0.5;}
			.ht-multilanguage-form .btn-language-form.current{opacity: 1;}
        </style>
        <script type="text/javascript">
            ...
            function generateMultilanguageElements(d){
                ...
                d.find('form').each(function(i,form){
                    f = jQuery(form);
                    if(f.find('.multilanguage-input, .term-description-wrap #description, .term-description-wrap #tag-description, .wp_attachment_details #attachment_content, .wp_attachment_details #attachment_caption').length > 0){
                        f.find('.multilanguage-input, .term-description-wrap #description, .term-description-wrap #tag-description, .wp_attachment_details #attachment_content,  .wp_attachment_details #attachment_caption').each(function(j,element){
                            e = jQuery(element);
                            ...
                            for(i = 0; i < languages.length; i++){
                                ...
                                switch(this.tagName.toLowerCase()){
                                    case 'input':
                                        html_input = '<input class="' + (e.attr('class') ? jQuery.trim(e.attr('class').replace('multilanguage-input','')):'') + '" value="'+value+'" type="text" lang="'+languages[i]+'" id="'+languages[i]+'_'+e.attr('id')+'" style="'+e.attr('style')+'" />';
                                    break;
                                    case 'textarea':
                                        html_input = '<textarea class="' + (e.attr('class') ? jQuery.trim(e.attr('class').replace('multilanguage-input','')):'') + ' " ' + ((e.attr('cols') > 0) ? 'cols="'+e.attr('cols')+'"' : '') + 'rows="'+e.attr('rows')+'" lang="'+languages[i]+'" id="'+languages[i]+'_'+e.attr('id')+'" style="'+e.attr('style')+'">'+value+'</textarea>';
                                    break;
                                }
                                html_flag = '<img alt="'+languages[i]+'" src="<?php bloginfo('wpurl'); ?>/wp-content/plugins/mqtranslate/flags/'+flags[i]+'" />';
                ...
                function prepareMultilanguageData(form){
                    ...
                    form.find('.multilanguage-input, .term-description-wrap #description, .term-description-wrap #tag-description, .wp_attachment_details #attachment_content,  .wp_attachment_details #attachment_caption').each(function(i, e){
                        content = '';
                        for(i=0; i < languages.length; i++){
                    ...
                    form.find('.ht-multilanguage-input').each(function(i,e){
						content = '';
                        for(i=0; i < languages.length; i++){
                        	var textarealang = jQuery(e).parent().find('textarea#'+languages[i]+'_'+jQuery(e).attr('id')+'[aria-hidden=false]');
                       		if(textarealang.length > 0 && jQuery.trim(val) != ''){
                        		content += '<!--:'+languages[i]+'-->'+val+'<!--:-->';
                        	} else {
	                            val = jQuery(e).parent().find('iframe#'+languages[i]+'_'+jQuery(e).attr('id')+'_ifr').contents().find('body').html();
								if(jQuery.trim(val) != ''){
	                            	content += '<!--:'+languages[i]+'-->'+val+'<!--:-->';
	                           	}
                           	}
                ...
                jQuery(document).ready(function(){
	                ...
	                jQuery('.ht-multilanguage-form > img.btn-language-form').each(function(){
						jQuery('<li class="bnt-language">'+jQuery(this)[0].outerHTML+'</li>').appendTo(jQuery(this).parent().find('.switch-language-input'));
						jQuery(this).remove();
					});
					jQuery('.ht-multilanguage-form .switch-language-input .bnt-language').on('click',function(){
						jQuery(this).parent().find('img.btn-language-form').removeClass('current');
						jQuery(this).find('img.btn-language-form').addClass('current');
						jQuery(this).parent().parent().find('.language-form').removeClass('current');
						jQuery(this).parent().parent().find('.language-form.'+jQuery(this).find('img.btn-language-form').attr('data-languageform')).addClass('current');
						return false;
					});
                   ...
                   /* jQuery('form.multilanguage-form').live('submit',function(){
                    ...
                    jQuery('.ht-multilanguage-form').parents('form').live('submit',function(){
                        prepareMultilanguageData(jQuery(this));
                    });
